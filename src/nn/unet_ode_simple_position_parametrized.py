...
from .nn_utils import ConvBlock, ResConvBlock, PPODEfunc, ODEBlock
import torch


class PPODEUNetSimple(BaseNetwork):
    ...
    def __init__(self,
        ...
        for d in range(self.depth):
            self.ode_list.append(ODEBlock(PPODEfunc(dim=n_f * 2 ** d)))
        ...
        self.ode_bottleneck = ODEBlock(PPODEfunc(dim=n_f * 2 ** self.depth))
